...
"""
CUDA_ID = 'cuda:3' if torch.cuda.is_available() else 'cpu'
...
TME_data_folder = os.path.join(root_data_folder, 'TMEinfo')
...
# TME features
tme_feature_file = os.path.join(TME_data_folder, '574tme_features.csv')
pathway_file = os.path.join(TME_data_folder, 'TMEGenes_pathways.gmt')
# TME in cancer features
tme_can_feature_file = os.path.join(TME_data_folder, '481tme_can_features.csv')
pathway_can_file = os.path.join(TME_data_folder, 'TMEGenes_cancer_pathways.gmt')
...
drug_smiles = numpy.array(object=object)