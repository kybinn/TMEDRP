...
def get_unlabeled_dataloaders(gex_features_df,TMEgenes,TME_cancer_genes, seed, test_ratio, batch_size):
    ...
    # ccle_sample_info_df.OncotreePrimaryDisease.value_counts() 是得到每个癌症类别的样本数量；
    ...
    train_tcga_dataset = TensorDataset(torch.from_numpy(tcga_df.drop(columns='Tissue').values.astype('float32')),
                                       torch.from_numpy(tcga_df.loc[:, ~tcga_df.columns.isin(TMEgenes)].drop(columns='Tissue').values.astype('float32')),
                                       torch.from_numpy(tcga_df.loc[:, TMEgenes].values.astype('float32')), 
                                       torch.from_numpy(tcga_df.loc[:, TME_cancer_genes].values.astype('float32')), 
    ...
    train_ccle_dataset = TensorDataset(torch.from_numpy(ccle_df.drop(columns='Tissue').values.astype('float32')),
                                       torch.from_numpy(ccle_df.loc[:, ~ccle_df.columns.isin(TMEgenes)].drop(columns='Tissue').values.astype('float32')),
                                       torch.from_numpy(ccle_df.loc[:, TMEgenes].values.astype('float32')), 
                                       torch.from_numpy(ccle_df.loc[:, TME_cancer_genes].values.astype('float32')), 
    ...
    test_tcga_dateset = TensorDataset(torch.from_numpy(test_tcga_df.drop(columns='Tissue').values.astype('float32')),
                                       torch.from_numpy(test_tcga_df.loc[:, ~test_tcga_df.columns.isin(TMEgenes)].drop(columns='Tissue').values.astype('float32')),
                                       torch.from_numpy(test_tcga_df.loc[:, TMEgenes].values.astype('float32')),
                                       torch.from_numpy(test_tcga_df.loc[:, TME_cancer_genes].values.astype('float32')),
    ...
    test_ccle_dateset = TensorDataset(torch.from_numpy(test_ccle_df.drop(columns='Tissue').values.astype('float32')),
                                       torch.from_numpy(test_ccle_df.loc[:, ~test_ccle_df.columns.isin(TMEgenes)].drop(columns='Tissue').values.astype('float32')),
                                       torch.from_numpy(test_ccle_df.loc[:, TMEgenes].values.astype('float32')),
                                       torch.from_numpy(test_ccle_df.loc[:, TME_cancer_genes].values.astype('float32')),
                                      torch.from_numpy(np.array(itemgetter(*test_ccle_df.Tissue)(config.tissue_map))))
    
    train_tcga_dataloader = DataLoader(train_tcga_dataset, batch_size=batch_size, shuffle=True)
    test_tcga_dataloader = DataLoader(test_tcga_dateset, batch_size=batch_size, shuffle=False)
    train_ccle_dataloader = DataLoader(train_ccle_dataset, batch_size=batch_size, shuffle=True)
...
def get_ccle_multi_labeled_dataloader(gex_features_df,TMEgenes,TME_cancer_genes, batch_size, drug, seed, measurement,
    ...
    ccle_labeled_samples = gex_features_df.index.intersection(ccle_target_df.index)  #715个具有表达值和药物反应值的细胞系的ID
    ...
    config.drug_smiles = SMILES.loc[drugs_to_keep]['Isosmiles']
    ...
    for train_index, test_index in kfold.split(ccle_labeled_feature_df.values, ccle_labels.values):
        ...
        train_labeled_ccle_df_cancer, test_labeled_ccle_df_cancer, = ccle_labeled_feature_df.loc[:, ~ccle_labeled_feature_df.columns.isin(TMEgenes)].values[train_index],\
                                                 ccle_labeled_feature_df.loc[:, ~ccle_labeled_feature_df.columns.isin(TMEgenes)].values[test_index]
        train_labeled_ccle_df_tme, test_labeled_ccle_df_tme, = ccle_labeled_feature_df.loc[:, TMEgenes].values[train_index],\
                                                    ccle_labeled_feature_df.loc[:, TMEgenes].values[test_index]
        train_labeled_ccle_df_tme_can, test_labeled_ccle_df_tme_can, = ccle_labeled_feature_df.loc[:, TME_cancer_genes].values[train_index],\
                                                    ccle_labeled_feature_df.loc[:, TME_cancer_genes].values[test_index]
        ...
        train_labeled_ccle_dateset = TensorDataset(
            ...
            torch.from_numpy(train_labeled_ccle_df_cancer.astype('float32')),   
            torch.from_numpy(train_labeled_ccle_df_tme.astype('float32')),      
            torch.from_numpy(train_labeled_ccle_df_tme_can.astype('float32')),     
        ...
        test_labeled_ccle_df = TensorDataset(
            ...
            torch.from_numpy(test_labeled_ccle_df_cancer.astype('float32')),  
            torch.from_numpy(test_labeled_ccle_df_tme.astype('float32')),       
            torch.from_numpy(test_labeled_ccle_df_tme_can.astype('float32')),
        ...
        train_labeled_ccle_dataloader = DataLoader(train_labeled_ccle_dateset, batch_size=batch_size, shuffle=True)
...
def get_tcga_multi_labeled_dataloaders(gex_features_df,TMEgenes,TME_cancer_genes, drug, batch_size):
    ...
    # config.tissue_map{'Adrenal Gland': 0, 'Lung': 1, 'Head and Neck ': 2, 'Prostate': 3, 'Thyroid': 4, 'Brain': 5, ...}

    tcga_labeled_df = tcga_labeled_df.drop(columns='Cancer').loc[intersection_index] 
    tcga_labeled_df = tcga_labeled_df[drug]
    tcga_labeled_gex_feature_df = tcga_gex_feature_df.loc[intersection_index]  # 1186 rows x 1306 columns]
    masked_labels = (~np.array(tcga_labeled_df.isnull())).astype(int)       # (1186, 9)
    print("number of non-NA labels: {}".format(np.sum(masked_labels))) 
    # Binarize labels  
    Binarize={'Complete Response':1.0, 'Partial Response':1.0, 'Clinical Progressive Disease':0.0,
              'Stable Disease':0.0, np.nan:-1.0}
    
    tcga_labeled_df = tcga_labeled_df.replace(Binarize, regex=False) #1186 rows x 9 columns  
    # Dataloading
    tcga_data = TensorDataset(torch.from_numpy(tcga_labeled_gex_feature_df.values.astype('float32')), 
                  torch.from_numpy(tcga_labeled_gex_feature_df.loc[:, ~tcga_labeled_gex_feature_df.columns.isin(TMEgenes)].values.astype('float32')),
                  torch.from_numpy(tcga_labeled_gex_feature_df.loc[:, TMEgenes].values.astype('float32')),
                  torch.from_numpy(tcga_labeled_gex_feature_df.loc[:, TME_cancer_genes].values.astype('float32')),
...
def get_multi_labeled_dataloader(gex_features_df,TMEgenes, TME_cancer_genes,drug, seed, batch_size, ccle_measurement,
                                 threshold_gdsc, threshold_label, n_splits=5):
